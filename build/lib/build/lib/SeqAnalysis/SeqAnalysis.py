...
def filter_by_taxa(record_dict,
    ...
    SeqIO.write(record_generator_by_id(record_dict, filtered_out_id_list), "filtered_out_by_taxa.gb", output_type)
    number_of_retained = len(filtered_id_list)
    print("Retained %i records" % number_of_retained)

    if return_record_dict:
        return record_generator_by_id(record_dict, filtered_id_list)


def split_by_taxa(record_dict, taxa_list, output_suffix, output_type="genbank"):
    print("Spliting taxa...")
    taxa_dict = {}
    for taxa in taxa_list:
        taxa_dict[taxa] = []
    taxa_dict["outfiltered"] = []
    ...
    for record_id in record_dict:
        ...
        for taxa in taxa_list:
            ...
            taxa_dict["outfiltered"].append(record_id)
...
def get_alignment(input_file, output_file="alignment.fasta", aligner_options=None, aligner="MAFFT", threads=6):
    if aligner == "MAFFT":
        if aligner_options:
            aligner_string = "mafft --thread %i " % threads + aligner_options + " " + input_file + " > " + output_file
        else:
            aligner_string = "mafft --thread %i --globalpair --maxiterate 1000 " % threads + input_file + " > " + output_file
...
def split_mitochondrion_genome_by_genes(record_dict, check_presense_coding_rRNA_genes=False):
    ...
    write_data(region_dict, "nucleotide", white_list=regions_set)
    write_data(protein_dict, "protein", white_list=protein_set)
...
def get_random_species_genomes(record_dict, count_species_file, output_file, output_type="fasta"):
    ...
    for line in fd:
        ...
        random_ids.append(id_list[random_number-1])
    fd.close()
