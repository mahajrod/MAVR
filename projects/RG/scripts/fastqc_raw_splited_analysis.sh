...
for SAMPLE in ${SAMPLE_LIST[@]};
    ...
    mkdir -p ${FASTQC_STAT_RAW_SPLITED_DIR}/${SAMPLE_GROUP} ${FASTQC_STAT_RAW_SPLITED_DIR}/${SAMPLE_GROUP}/${SAMPLE};

    NUMBER_OF_FILES=`ls ${RAW_READS_DIR}/${SAMPLE_GROUP}/${SAMPLE}/* | wc -l`

    OUTPUT_DIR=${FASTQC_STAT_RAW_SPLITED_DIR}/${SAMPLE_GROUP}/${SAMPLE}/

    echo "Starting FastQC analysis"
    echo "    ${NUMBER_OF_FILES} files"

    FASTQC_STRING="${FASTQC_DIR}/fastqc -k 10 --nogroup -t ${NUMBER_OF_FILES} -o ${OUTPUT_DIR} ${RAW_READS_DIR}/${SAMPLE_GROUP}/${SAMPLE}/*"
    echo ${FASTQC_STRING}
