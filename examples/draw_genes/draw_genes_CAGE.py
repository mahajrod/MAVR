...
for sample in sample_list:
    samples_dict[sample] = [[], []]
    with open(CAGE_dir + sample + "/nxf_histo_size_3.t", "r") as in_fd:
        for line in in_fd:
            line_list = line.strip().split()
            if line_list[0] == "-1":
                samples_dict[sample][0].append(int(line_list[1]))
                samples_dict[sample][1].append(float(line_list[2]) * float(1000000) / float(sample_size[sample]))
...
borders = [(10846400, 10847400), (10836600, 10837600)]

ymax = max([max(samples_dict[sample][1]) for sample in samples_dict])

figure = plt.figure(1, (5, 7), dpi=300)
subplot_index = 1
for sample in samples_dict:
    for region_index in [0, 1]:
        ...
        if region_index == 0:
            ...
            plt.text(-0.3, 0.5, sample, rotation=90, fontweight="bold", transform=subplot.transAxes, fontsize=9,
...
for extension in ".jpg", ".svg", ".png":
    plt.savefig("CAGE_sbr_locus_TPM%s" % extension)
