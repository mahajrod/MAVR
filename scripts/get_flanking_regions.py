...
parser.add_argument("-l", "--left", action="store", dest="left",
                    help="The number of base pairs that a flank should start from orig. start coordinate.- (Integer) or (Float, e.g. 0.1) if used with -pct.")
...
parser.add_argument("--pct", action="store_true", dest="fraction_mode",
                    help="Define -l and -r as a fraction of the feature's length.E.g. if used on a 1000bp feature, -l 0.50,will add 500 bp \"upstream\".  Default = false")

parser.add_argument("-s", action="store_true", dest="use_strand",
                    help="Define -l and -r based on strand. E.g. if used, -l 500 for a negative-stranded feature, it will start the flank 500 bp downstream.  Default = false.")

args = parser.parse_args()

record_dict = SeqIO.index_db("temp_index.idx", [args.input_fasta], format="fasta")

get_lengths(record_dict, out_file="fasta_lengths.t", write=True)
